...
class LineItemsController < ApplicationController
  ...
  def create
    ...
    if @line_item.save
      respond_to do |format|
        format.html { redirect_to quote_path(@quote), notice: "Item #{t('created')}" }
        format.turbo_stream { flash.now[:notice] = "Item #{t('created')}" }
      end
  ...
  def update
    if @line_item.update(line_item_params)
      respond_to do |format|
        format.html { redirect_to quote_path(@quote), notice: "Item #{t('updated')}" }
        format.turbo_stream { flash.now[:notice] = "Item #{t('updated')}" }
      end
  ...
  def destroy
    @line_item.destroy
    respond_to do |format|
      format.html { redirect_to quote_path(@quote), notice: "Item #{t('destroyed')}" }
      format.turbo_stream { flash.now[:notice] = "Item #{t('destroyed')}" }
    end
  end
